/**
 * Página Contas a Receber – Lista de receitas com filtros e paginação.
 */
window.ContasReceberPage = (function() {
  ...
  function renderTabela(filtros) {
    ...
    const pagInfo = document.getElementById('contas-receber-pag-info');
    const btnPrev = document.getElementById('contas-receber-pag-prev');
    const btnNext = document.getElementById('contas-receber-pag-next');
      ...
      tbody.innerHTML = itens.map(l => {
        ...
        return `
          <tr>
            ...
            <td>${nomeCat}</td>
            <td class="success">${Utils.formatCurrency(l.valor)}</td>
  ...
  function excluir(id) {
    const item = receitas.find(l => l.id === id);
  ...
  function abrirModalEdicao(id) {
    ...
    const html = `
      <form id="form-editar-receita" novalidate>
    ...
    document.getElementById('form-editar-receita').addEventListener('submit', function(e) {
      ...
      if (dataPagamento) {
        if (!Utils.isValidDate(dataPagamento)) { UI.showToast('Data de recebimento inválida.', 'error'); return; }
        if (dataPagamento < data) { UI.showToast('A data de recebimento não pode ser anterior à data do lançamento.', 'error'); return; }
      }
  ...
  function render() {
    ...
    main.innerHTML = `
      <div class="card">
        ...
        <div class="pagination">
          <div class="pagination-info" id="contas-receber-pag-info"></div>
          <div class="pagination-buttons">
            <button id="contas-receber-pag-prev" class="btn"><i class="fas fa-chevron-left"></i> Anterior</button>
            <button id="contas-receber-pag-next" class="btn">Próxima <i class="fas fa-chevron-right"></i></button>
          </div>
        </div>
      </div>
    `;
    ...
    document.getElementById('contas-receber-pag-prev').addEventListener('click', () => irParaPagina(-1));
    document.getElementById('contas-receber-pag-next').addEventListener('click', () => irParaPagina(1));
